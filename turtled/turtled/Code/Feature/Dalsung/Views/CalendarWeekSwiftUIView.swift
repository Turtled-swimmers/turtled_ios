        ...
        HStack(spacing:22) {
                   ForEach(weekDates, id: \.self) { date in
                       ...
                       if isLastWeek() && (Int(date.date.day)! <= 7 || date.date.day == "1") {
                           CircularProgressBar(progress: 0, dateText: "  ", today: false, weekDates: date.weekdayText, isSelected: false, isDone: date.isDone)
                               .padding(.top, 4)
                               .onTapGesture {
                                   selectedDate = date
                               }
                           ...
                           if isLastOne(in: weekDates, current: date) {
                               CircularProgressBar(progress: 0.9, dateText: date.dayText, today: isSameDate(date.date, Date()), weekDates: date.weekdayText, isSelected: isSelectedDate,isDone: date.isDone)
                                   .padding(.top, 4)
                                   .onTapGesture {
                                       selectedDate = date
                                   }
                           ...
                           CircularProgressBar(progress: 0.2, dateText: date.dayText, today: isSameDate(date.date, Date()), weekDates: date.weekdayText, isSelected: isSelectedDate,isDone: date.isDone)
                               .padding(.top, 4)
                               .onTapGesture {
                                   selectedDate = date
                               }
...
struct CalendarWeekSwiftUIView_Previews: PreviewProvider {
    ...
    static var sampleWeek: [DayModel] {
        ...
        return (0..<7).map { offset in
            ...
            return DayModel(date: date, isDone: false) // isDone 속성을 false로 초기화했습니다.
        }
    }
}
